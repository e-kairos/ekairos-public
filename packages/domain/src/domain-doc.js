...
function parseDomainHeading(title) {
...
function parseMetaLines(content) {
...
function parseEntities(content) {
    ...
    const startEntity = (name, summary) => {
        flush();
        currentName = name;
...
function parseSubdomains(sections, startIndex) {
    const subdomains = {};
    let i = startIndex + 1;
...
function parseMarkdownDomainDoc(markdown) {
    ...
    const overviewSection = sections.find((section) => section.level === 2 && normalizeTitle(section.title) === "overview");
    ...
    const responsibilitiesSection = sections.find((section) => section.level === 2 && normalizeTitle(section.title) === "responsibilities");
    if (responsibilitiesSection) {
        doc.responsibilities = parseList(responsibilitiesSection.content);
    }
    const navigationSection = sections.find((section) => section.level === 2 && normalizeTitle(section.title) === "navigation");
    if (navigationSection) {
        doc.navigation = parseList(navigationSection.content);
    }
    const entitiesSection = sections.find((section) => section.level === 2 && normalizeTitle(section.title) === "entities");
    if (entitiesSection) {
        doc.entities = parseEntities(entitiesSection.content);
    }
    const subdomainsSectionIndex = sections.findIndex((section) => section.level === 2 && normalizeTitle(section.title) === "subdomains");
    ...
    for (const section of sections) {
        ...
        if (reservedTitles.has(normalized))
            continue;
        if (!section.content.trim())
            continue;
...
function parseYamlDomainDoc(markdown) {
...
export function parseDomainDoc(markdown) {
    if (!markdown)
        return null;
...
export function filterDomainDoc(doc, filter) {
...
function renderEntityEntry(key, value) {
...
function pushEntitiesSection(lines, heading, entities, includeEntities = true) {
    if (!includeEntities || !entities || Object.keys(entities).length === 0)
        return;
    lines.push("");
    lines.push(heading);
...
function pushSubdomain(lines, name, sub, includeEntities = true) {
    lines.push("");
    lines.push(`### ${name}`);
    if (sub.summary)
        lines.push(sub.summary.trim());
    if (sub.responsibilities?.length) {
        lines.push("");
        lines.push("#### Responsibilities");
        for (const item of sub.responsibilities)
            lines.push(`- ${item}`);
    }
    if (sub.navigation?.length) {
        lines.push("");
        lines.push("#### Navigation");
        for (const item of sub.navigation)
            lines.push(`- ${item}`);
    }
    if (includeEntities) {
...
export function renderDomainDoc(doc, options) {
