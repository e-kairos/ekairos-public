...
await setupInstantTestEnv("dataset-transform-wrapper")
if (hasInstantAdmin()) {
...
describeInstant("transformDataset wrapper (sample.csv)", () => {
  it("transforms parsed dataset into a new schema and returns preview rows", async () => {
    ...
    const result = await transformDataset({
      datasets: [{ id: sourceDatasetId, description: "Source CSV rows" }],
      outputSchema,
      description: [
        "Sea A = Dominio('dataset').",
        "Transformar filas: code -> sku, description -> name, price -> priceUsd.",
        "Calcular priceWithTaxUsd = round(priceUsd * 1.1, 2).",
      ].join("\n"),
    })
    ...
    const jsonlText = await getOutputJsonlText(adminDb, result.datasetId)
