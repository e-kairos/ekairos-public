...
> = {
  chatbot: {
    ...
    description:
      "Conversational primitives adapted to persisted context events and context hydration.",
  ...
  voice: {
    ...
    description:
      "Audio input/output primitives that map voice interactions to context items and parts.",
...
export function LandingClient(props: LandingClientProps) {
  const [previewText, setPreviewText] = useState("Context preview running with persisted events.");
    ...
    <div className="container">
      <header className="card hero">
        <div className="pill-row">
          ...
          <span className="pill">Context + Domain + InstantDB</span>
          <span className="pill">{props.catalog.length} elements</span>
        </div>

        <h1 className="hero-title">Ekairos Context Elements Registry</h1>
        <p className="hero-subtitle">
          Landing and docs for a Context-first component catalog inspired by AI Elements.
...
{`npx shadcn@latest add http://localhost:3040/api/registry/conversation.json`}
      ...
      <section className="card section">
        <div className="section-head">
          <div>
            ...
            <p className="section-desc">
              ...
              live context-backed preview with real data.
            </p>
          </div>
        ...
        >
          ...
          <label style={{ display: "grid", gap: 6 }}>
            <span className="section-desc">context state</span>
            <select
              value={previewState}
              onChange={(event) =>
      ...
      {categoryOrder.map((category) => {
        ...
        const meta = categoryMeta[category] ?? {
          title: category,
          description: "Context elements section.",
      ...
      <section id="context" className="section card">
        <div className="section-head">
          <div>
            <h2 className="section-title">Context Runtime Integration</h2>
            <p className="section-desc">
              Context snapshots are persisted in InstantDB and exposed through domain runtime.
        ...
        <div className="showcase-grid">
          <article className="context-strip">
            <h3>Context Snapshot API</h3>
            <p>Read persisted context state from the registry app runtime.</p>
            <pre className="cmd">{`GET /api/context/<contextKey>?orgId=<orgId>&ensure=1`}</pre>
          </article>
          <article className="context-strip">
            <h3>React Hook</h3>
            <p>Use a clean hook API shared with the context package.</p>
            <pre className="cmd">
{`const context = useContext({
  contextKey: "preview-conversation",
  orgId: "org_preview",
  ensure: true,
  refreshMs: 1200,
});`}
