...
if (process.platform === "win32") {
  ...
  if (await isPortInUse(PORT)) {
    console.error(`[story-workflow-smoke] Port ${PORT} is still in use after kill attempt.`);
    process.exit(1);
  }
} else if (await isPortInUse(PORT)) {
  console.error(
    `[story-workflow-smoke] Port ${PORT} is in use. Free it manually and rerun pnpm test:e2e.`,
  );
  process.exit(1);
}
...
if (result.error) {
  console.error("[story-workflow-smoke] Failed to run Playwright:", result.error);
  process.exit(1);
}
