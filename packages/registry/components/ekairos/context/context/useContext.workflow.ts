...
function makeStorageKey(apiUrl: string) {
  const prefix = `ekairos-context:context:${apiUrl}`;
...
export function useContext(db: any, opts: UseContextOptions): ContextValue {
  ...
  const {
    ...
    // CRITICAL: persisted `event_items` ids are Instant entity ids, which must be UUIDs.
    ...
    onData: (data: any) => {
      ...
      if (data.type === "data-context-id") {
        ...
        // No hijack: only update selected context if user is not browsing another context.
