...
function parseFileAndLine(rgLine, repoRoot) {
  const match = /^(.*):(\d+):(.*)$/.exec(rgLine);
  if (!match) {
    return { file: rel(rgLine, repoRoot), line: null, raw: rgLine };
  }
  const abs = path.resolve(match[1]);
...
function renderMarkdown(report) {
  ...
  lines.push(`- Reactor matches (informational): **${report.summary.reactorMatches}**`);
...
async function main() {
  ...
  const scanGroup = async (title, patterns) => {
    ...
    for (const entry of patterns) {
      const lines = await rgLines(scanRoot, entry.pattern);
