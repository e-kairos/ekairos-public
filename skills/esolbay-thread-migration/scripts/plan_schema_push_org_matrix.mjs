...
async function resolveSchema(schemaFile) {
  ...
  if (ext === ".ts" || ext === ".mts" || ext === ".tsx") {
    ...
    try {
      ...
      tsxError = new Error(`tsx import failed for schema: ${resolved}`);
    }
    ...
    if (helper && typeof helper.schemaTypescriptFileToInstantSchema === "function") {
      try {
        ...
        return { schema, schemaSource: resolved, schemaType: "typescript:platform-fallback" };
      } catch {
        // Continue to final error.
      }
    }

    if (tsxError) throw tsxError;
    throw new Error(`Failed to resolve TypeScript schema: ${resolved}`);
  }
...
async function planForApp(params) {
  const endpoint = `${params.instantApiUri}/dash/apps/${encodeURIComponent(params.appId)}/schema/plan`;
...
async function main() {
  ...
  const orgSeeds =
    orgFilter.size > 0
      ? Array.from(orgFilter).map((orgId) => ({ id: orgId }))
      : await listClerkOrganizations({
          clerkApiUri,
          clerkSecretKey,
          limit: 100,
        });
  ...
  for (const orgRow of orgSeeds) {
    ...
    let detail = null;
    try {
      detail = await getClerkOrganization(clerkApiUri, clerkSecretKey, orgId);
    } catch (error) {
