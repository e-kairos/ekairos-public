...
1. `verify_env`
2. `preflight_scan`
3. `plan_story_context_model_migration`
...
5. Transform source dataset to context model
...
- `artifacts/reports/story-context-model-plan.json`
- `artifacts/reports/story-context-model-plan.md`
...
- `conversation_history_orphan` (messages exist in `conversation_messages`/`whatsapp_messages` but no `context_contexts` + `context_items` for award timeline)
...
3. Upsert `context_contexts` + `context_contexts`.
4. Backfill `context_items` from `whatsapp_messages` in matching conversation.
5. Link `conversation_messages` -> `context` + `event`.
6. Verify with the same query shape used by timeline (`context_items where context.key = award_<awardId>`).

Command:

```powershell
node scripts/migrate_conversation_history_to_context.mjs --org <clerk-org-id> --tender-id <tender-uuid> --run-id <run-id>
```
...
- `transform_story_to_context_dataset.mjs`
- `apply_instant_tx.mjs`
- `verify_migration.mjs`
- `migrate_conversation_history_to_context.mjs`
...
node scripts/transform_story_to_context_dataset.mjs --input .\artifacts\datasets\source.json --output .\artifacts\transformed\story-context-target.json --org <clerk-org-id> --env production --run-id <run-id>
node scripts/apply_instant_tx.mjs --app-id <instant-app-id> --token <instant-admin-token> --input .\artifacts\transformed\story-context-target.json --dry-run --org <clerk-org-id> --env production --run-id <run-id>
...
- Endpoint and agent paths are context-backed with reactor configuration.
