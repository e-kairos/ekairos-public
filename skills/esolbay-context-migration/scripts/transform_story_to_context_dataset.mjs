...
function transformStoryToContext(sourcePayload) {
  ...
  const contextToContext = new Map();
  ...
  for (const context of legacyContexts) {
    ...
    const contextId =
      firstLinkedId(context?.context) ||
      safeId(context?.contextId) ||
      `context_${contextId}`;
    const key = typeof context?.key === "string" ? context.key : undefined;
    if (key) contextKeyToId.set(key, contextId);
    contextToContext.set(contextId, contextId);

    records.push(
      createRecord("context_contexts", contextId, {
    ...
    linkIntents.push(createLinkIntent("context_contexts", contextId, { context: contextId }));
  }
  ...
  for (const execution of legacyExecutions) {
    ...
    const contextId =
      (contextId && contextToContext.get(contextId)) ||
      firstLinkedId(execution?.context) ||
      safeId(execution?.contextId) ||
      "";
    ...
    records.push(
      createRecord("context_executions", executionId, {
    ...
    if (contextId) {
      linkIntents.push(createLinkIntent("context_executions", executionId, { context: contextId }));
    } else {
    ...
    if (triggerEventId) {
      linkIntents.push(createLinkIntent("context_executions", executionId, { trigger: triggerEventId }));
    }
    if (reactionEventId) {
      linkIntents.push(createLinkIntent("context_executions", executionId, { reaction: reactionEventId }));
    }
    if (contextId) {
      linkIntents.push(createLinkIntent("context_contexts", contextId, { currentExecution: executionId }));
    }
  }
  ...
  for (const event of legacyEvents) {
    ...
    records.push(
      createRecord("context_items", eventId, {
        channel:
    ...
    if (contextId) {
      linkIntents.push(createLinkIntent("context_items", eventId, { context: contextId }));
    } else {
    ...
    if (executionId) {
      linkIntents.push(createLinkIntent("context_items", eventId, { execution: executionId }));
    }
  }
  ...
  for (const step of legacySteps) {
    ...
    records.push(
      createRecord("context_steps", stepId, {
    ...
    if (executionId) {
      linkIntents.push(createLinkIntent("context_steps", stepId, { execution: executionId }));
    } else {
  ...
  for (const part of legacyParts) {
    ...
    records.push(
      createRecord("context_parts", partId, {
        key,
        stepId,
        idx,
        type:
    ...
    linkIntents.push(createLinkIntent("context_parts", partId, { step: stepId }));
  }
...
async function main() {
  ...
  await writeMigrationAudit({
    runId,
    script: "transform_story_to_context_dataset",
    stage: "snapshot",
    orgId,
    envName,
    payload: {
  ...
  const transformed = transformStoryToContext(source);
