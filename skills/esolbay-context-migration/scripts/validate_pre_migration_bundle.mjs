...
async function main() {
  ...
  const transformOutputFile = path.join(skillRoot, "artifacts", "runs", runId, "02-transform", "story-context-target.json");
  ...
  const transformCmd = await runNode(
    path.join(scriptsDir, "transform_story_to_context_dataset.mjs"),
