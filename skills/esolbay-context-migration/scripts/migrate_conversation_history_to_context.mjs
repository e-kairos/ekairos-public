...
async function main() {
  ...
  const existingContextRes = await adminQuery(appId, adminToken, {
    context_contexts: {},
    context_items: { whatsappMessages: {}, context: {}, context: {} },
  });
  ...
  const conversationMessages = Array.isArray(conversationRes?.conversation_messages)
    ...
    : [];
  const existingContexts = Array.isArray(existingContextRes?.context_contexts) ? existingContextRes.context_contexts : [];
  const existingItems = Array.isArray(existingContextRes?.context_items) ? existingContextRes.context_items : [];
  ...
  for (const award of awards) {
    ...
    steps.push(["link", "context_contexts", contextId, { context: contextId }]);
  ...
  const report = {
    ...
    verifyCounts: {
      context_contexts: Array.isArray(verify?.context_contexts) ? verify.context_contexts.length : 0,
      context_items: Array.isArray(verify?.context_items) ? verify.context_items.length : 0,
    },
    files: {
      sourceFile,
      txFile,
