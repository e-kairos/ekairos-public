...
- Confirm `@ekairos/events` target version published from workspace.
...
- Critical tests for session/context paths pass.
- Preflight scan (`context + reactor`) report exists and is attached.
- Story->Context model plan report exists (`story-context-model-plan.json` + `.md`).
- Endpoint routes in migration scope are context-backed (no runtime `@ekairos/story` usage).
...
- Legacy model migration completed:
  - `context_contexts` -> `context_contexts` + `context_contexts`
  - `context_events` -> `context_items`
  - `story_executions` -> `context_executions`
  - `story_steps` -> `context_steps`
  - `story_parts` -> `context_parts`
...
3. Verify domain queries and context execution.
